...
const drawLine = (ctx: CanvasRenderingContext2D, line: string, x: number, y: number, letterSpacing: number, align: 'left' | 'center' | 'right' = 'center') => {
  const chars = Array.from(line);
  let cursor = x;
  if (align === 'center') {
...
const buildTextCanvas = ({ container, width, height, dpr, props }: {
  ...
  lines.forEach((line, index) => {
    ...
    drawLine(ctx, line, x, startY + index * lineHeight, letterSpacing, align);
  });
...
const WarpText: React.FC<WarpTextProps> = ({
  ...
  useEffect(() => {
    ...
    mediaQuery?.addEventListener('change', onReducedMotion as any);
    ...
    return () => {
      ...
      mediaQuery?.removeEventListener('change', onReducedMotion as any);
